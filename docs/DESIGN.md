...
- Network: fixed HTTPS endpoints at site.api.espn.com, v1.rugby.api-sports.io, api.cricapi.com. Redirects disabled, 5-second connect and 12-second request timeout; 2 MiB per response; max four cricket pages; 55-second process watchdog. Provider response URLs/errors and keys are not passed to QML.
- Credentials: helper inherits only the two named credential variables it reads. No key UI or copying from another program's credential store.
...
- Failure states: missing helper, offline, HTTP/authentication/quota failures, unsupported competition/pagination and malformed data. Last-good scores survive with stale labels. First successful snapshot after a failure is silent. No fake live-data fallback.
